...
void function S2S_EntitiesDidLoad()
{
	...
	/*entity shop = CreatePropDynamic( CHEST_INTERACTABLE_MODEL, s2s_mover.GetOrigin(), <0, RandomFloatRange(0, 180), 0>, 6 )
    shop.SetParent(s2s_mover)
    //shop.SetUsable()
    shop.SetScriptName("roguelike_shop")
    //shop.SetUsableByGroup( "pilot" )
    //DispatchSpawn( shop )
    Highlight_SetNeutralHighlight( shop, "roguelike_bounds" )

	shop = CreatePropDynamic( CHEST_INTERACTABLE_MODEL, s2s_mover.GetOrigin() + <500, 0, 00>, <0, RandomFloatRange(0, 180), 0>, 6 )
    shop.SetParent(s2s_mover)
    shop.SetUsable()
    shop.SetScriptName("roguelike_shop")
    //shop.SetUsableByGroup( "pilot" )
