...
bool function OnWeaponChargeLevelIncreased_titanweapon_sniper( entity weapon )
{
	...
	if (IsValid( weapon.GetWeaponOwner() ) && weapon.HasMod( "infinite_charge") )
	{
		maxLevel = 5 + Roguelike_GetItemCount( weapon.GetWeaponOwner(), "infinite_charge" )
	}
	if (level > maxLevel)
		return false

	if (weapon.HasMod("infinite_charge"))
	{
		if (level == maxLevel - 1)
			level = 5
			...
			level = 3
		else if (level != maxLevel) level = ((level - 1) % 2) + 1
		//print(level)
	}
...
function FireSniper( entity weapon, WeaponPrimaryAttackParams attackParams, bool playerFired )
{
	...
	if ( bolt )
	{
		...
		int infiniChargeStacks = 0
		if (IsValid( weaponOwner ))
		{
			infiniChargeStacks = Roguelike_GetItemCount( weaponOwner, "infinite_charge" )
			print(int( Roguelike_ReverseHyperbolicChanceFunc( 3 )(infiniChargeStacks) / 100.0 * weapon.GetWeaponSettingInt( eWeaponVar.damage_additional_bullets ) ))
		}
		bolt.s.bulletsToFire <- chargeLevel

		bolt.s.extraDamagePerBullet <- int( Roguelike_ReverseHyperbolicChanceFunc( 3 )(infiniChargeStacks) / 100.0 * weapon.GetWeaponSettingInt( eWeaponVar.damage_additional_bullets ) )
		bolt.s.extraDamagePerBullet_Titan <- int( Roguelike_ReverseHyperbolicChanceFunc( 3 )(infiniChargeStacks) / 100.0 * weapon.GetWeaponSettingInt( eWeaponVar.damage_additional_bullets_titanarmor ) )
...
void function OnWeaponStartZoomIn_titanweapon_sniper( entity weapon )
{
	...
	{
		if (Roguelike_GetItemCount( weapon.GetWeaponOwner(), "infinite_charge" ) > 0)
			weapon.AddMod( "infinite_charge" )
	}
	#endif
}
