...
function FireWeaponPlayerAndNPC( WeaponPrimaryAttackParams attackParams, bool playerFired, entity weapon )
{
	...
	if ( shouldCreateProjectile )
	{
		...
		{
			vector upVec = <0,0,0>
			if (weapon.HasMod("wide"))
				upVec = baseUpVec * file.wideBoltOffsets[index][0] * spreadFrac
			else upVec = baseUpVec * file.boltOffsets[index][0] * spreadFrac
			vector rightVec = <0,0,0>
			if (weapon.HasMod("wide"))
				rightVec = baseRightVec * file.wideBoltOffsets[index][1] * spreadFrac
			else rightVec = baseRightVec * file.boltOffsets[index][1] * spreadFrac
